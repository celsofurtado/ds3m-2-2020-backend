...
public class TokenService {
	...
	public String gerarToken(Authentication authentication) {
		...
		return Jwts.builder().setIssuer("API Instagames").setSubject(user.getId().toString()).setIssuedAt(hoje)
				.setExpiration(dateExpiration).signWith(SignatureAlgorithm.HS256, this.secret).compact();
	}
